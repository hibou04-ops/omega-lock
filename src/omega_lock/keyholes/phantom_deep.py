...
threshold/lookback/mode trio):
    Channel A (stronger; labels ±1):
        latent x[t], obs_a[t] = x[t] + ε_a
        labels_a[t] = +1 if x[t] > LABEL_Z else -1
        val_a[t] = mean(obs_a[t-window : t])
        fires_a = (val_a > alpha) if long_mode else (val_a < -alpha)
        reward_a = Σ labels_a[fires_a]
        Effectives: alpha, window, long_mode

    Channel B (weaker; labels ±0.6):
        latent y[t] (independent of x), obs_b[t] = y[t] + ε_b
...
_OBS_NOISE_A: float = 0.10          # std of obs_a = x + ε
_OBS_NOISE_B: float = 0.12          # std of obs_b = y + ε
...
class PhantomKeyholeDeep:
    ...
    def __post_init__(self) -> None:
        ...
        # --- Channel A: latent x, AR(1) with ρ_A ---
        ...
        # --- Channel B: latent y, AR(1) with ρ_B, independent innovations ---
    ...
    def param_space(self) -> list[ParamSpec]:
        return [
            ...
            # fire configs (α=1.0 → <20 fires/600 events, tripping KC-3).
    ...
    def evaluate(self, params: dict[str, Any]) -> EvalResult:
        ...
        if use_ema:
            # EMA with fixed α — use_ema=True turns on smoothing.
    ...
    def true_importance_ranking() -> list[str]:
        """Channel-A magnitudes (±1.0) dominate channel-B (±0.6), so
